...
func runPostCommit(repoRoot string, cfg config.Config) error {
	...
	// Parse agent session data (Claude only; other agents don't expose JSONL).
	var sessionPath string
	var sessionData *agent.SessionData
	if state.AgentName == "" || state.AgentName == "claude-code" {
		claudeDetector := claude.New()
		var sessErr error
		sessionPath, sessionData, sessErr = claudeDetector.FindLatestSession(repoRoot)
		if sessErr != nil {
			slog.Warn("could not read agent session", "error", sessErr)
		}
	}
	...
	// Use the detected agent name from pre-commit state when available.
	agentName := cfg.Agent
	if state.AgentName != "" {
